...
export default function Index() {
  ...
  const handleSearch = async (query) => {
    ...
    try {
      const weatherPayload = await fetchWeather(query);
      setWeatherData(weatherPayload);
      setLastQuery(query);
      setStatus("ready");
    } catch (error) {
      const message = error instanceof Error ? error.message : "Unable to fetch weather at the moment.";
